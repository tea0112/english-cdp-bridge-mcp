...
class TMWebDriver:
        ...
        if not session or not session.is_active():
            ...
            if not session or not session.is_active():
                ...
                if alive_sessions:
                    ...
                    _tlog(token, f"Session {session_id} not connected, auto-switching to latest active session: {session.id}")
    ...
    def _remote_cmd(self, cmd):
        try:
            ...
            raise ConnectionError("TMWebDriver master not running, see tmwebdriver_sop to start master")
    ...
    def set_session(self, url_pattern: str, token=None) -> bool:
        ...
        if not matched: return _tlog(token, f"Warning: No session found with URL containing '{url_pattern}'")
        if len(matched) > 1: _tlog(token, f"Warning: Multiple sessions found with URL containing '{url_pattern}', using first")
        ctx.default_session_id, info = matched[0]
        _tlog(token, f"Successfully set default session: {ctx.default_session_id}: {info['url']}")
