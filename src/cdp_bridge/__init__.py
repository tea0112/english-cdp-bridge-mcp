...
def main():
    ...
    if args.transport == "streamable-http" and args.host != "127.0.0.1" and args.host != "localhost":
        ...
        if args.host == "0.0.0.0":
            # When configured as all network interfaces, disable FastMCP DNS rebinding protection
            mcp.settings.transport_security = TransportSecuritySettings(
                enable_dns_rebinding_protection=False,
            )
        else:
            # Infer FastMCP DNS rebinding protection rules from --host: if --host is 192.168.0.1, the rule is "192.168.0.1:*"
            mcp.settings.transport_security = TransportSecuritySettings(
