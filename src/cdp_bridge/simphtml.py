...
except ImportError: log("[Error] BeautifulSoup4 is not installed. Please ask Agent to install BeautifulSoup4 before using web-related tools.")
...
function createEnhancedDOMCopy() {  
  ...
  function cloneNode(sourceNode, keep=false) {  
    ...
    try { if (sourceNode.matches && sourceNode.matches(':-webkit-autofill')) { clone.setAttribute('data-autofilled', 'true'); if (!sourceNode.value) clone.setAttribute('value', '⚠️Protected - See tmwebdriver_sop autofill section for extraction'); } } catch(e) {}
...
function analyzeNode(node, pPathType='main') {  
    ...
    console.log(`${isOverlay ? 'Overlay' : 'Partition'} container:`, node, `children count: ${children.length}`);  
  ...
  function handlePartitionContainer(childrenInfo, pathType) {  
    ...
    if (hasMainElement) {  
      ...
      for (let i = 1; i < childrenInfo.length; i++) {  
        ...
        if (child.node.textContent.trim().length > 200) isSecondary = true;  // P3: Keep if has substantial text content
  ...
  function handleOverlayContainer(childrenInfo, pathType) {  
    // elementFromPoint ground truth: let the browser tell us who's on top visually
...
js_findMainList = r'''function findMainList(startElement = null) {
        ...
        // Global scan: collect candidate containers, sorted by l1 + l2*0.1 (l2=grandchild count, captures tables etc.)
        const candidates = [];
        ...
        // Deduplicate: remove results overlapping >50% with higher-scored candidates
        const kept = [];
    ...
    function scoreContainer(container, items) {
        ...
        // Return low score if too few visible items
        if (visibleItems < 3) return 0;
        // Prevent outliers: ensure area doesn't exceed container
        totalItemArea = Math.min(totalItemArea, containerArea * 0.98);
        const areaRatio = totalItemArea / containerArea;
        // 3. Calculate item scores - use linear interpolation instead of steps
        // 3.2 Area ratio score - up to 40pts, continuous curve
        // Use sigmoid function for smoother scoring
        const areaScore = 40 / (1 + Math.exp(-12 * (areaRatio - 0.4)));

        // 3.3 Uniformity score - up to 20pts, continuous curve
        ...
        if (itemAreas.length >= 3) {
            ...
            // Exponential decay function, smaller cv = higher score
        ...
        // 3.4 Container size score - up to 15pts, continuous curve
        const viewportArea = window.innerWidth * window.innerHeight;
        ...
        let layoutScore = 0;
        if (items.length >= 3) {
            // Group coordinates and calculate row/col count
            ...
            // For single row/column give full marks; otherwise evaluate grid quality
        ...
        // Total score - still ~100 pts total
...
temp_monitor_js = """function startStrMonitor(interval) {  
    ...
    startStrMonitor(450);  
"""  
def start_temp_monitor(driver, token=None):
...
def get_main_block(driver, extra_js="", text_only=False, token=None):
    ...
    if text_only:
        page = re.sub(r' {2,}', ' ', page)           # consecutive spaces -> single space
        page = re.sub(r'^ +', '', page, flags=re.M)   # remove leading spaces
        page = re.sub(r'(\n\s*){3,}', '\n\n', page)   # 3+ blank lines -> 1 blank line
        return page.strip()
    return page
...
def smart_truncate(soup, budget, _depth=0):
    """Truncate soup in-place to approach budget characters.
    Strategy: pierce single-child elements to find fork point; if top3 can handle over, share proportionally, otherwise delete children from the tail."""
    CUT_THRESHOLD = 8000  # below this value, trim from tail directly; above, recurse to find fork point
    indent = '  ' * _depth
    def cut(ele, keep):
        ...
        over = len(s) - keep
        if over <= 0: return
        # Protect FAKE ELEMENT hint tags
    ...
    # Check if top 3 can absorb the over
    ...
    if top_total < over:
        # === top 3 can't handle it, delete children from tail ===
        removed = 0
        removed_count = 0
    ...
    # Filter out too-small kids (<10% of largest), let big ones absorb all
    ...
    # First print all allocation plan
    actions = []
    for i in tops:
        c, l = kids[i]
...
def execute_js_rich(script, driver, no_monitor=False, token=None):
    ...
    if response.get('newTabs'): rr['newTabs'] = response['newTabs']
        ...
        after = driver.get_session_dict(token=token)
        new_sids = {k: v for k, v in after.items() if k not in before_sids}
        if new_sids:
            newTabs = [{'id': k, 'url': v} for k, v in new_sids.items()]
            rr['newTabs'] = newTabs
            rr['suggestion'] = "The page has been reloaded and the new tabs above connected during execution."
    ...
    if not reloaded and len(newTabs) == 0:
        try:
            ...
            if change_count == 0 and not transients and len(newTabs) == 0:
                diff_summary += " (no page change detected)"
                rr['suggestion'] = "No significant page change detected"
        except:
